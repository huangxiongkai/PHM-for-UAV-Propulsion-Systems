...
| 模块 | 状态 | 说明 |
|------|------|------|
| DMA ADC 双缓冲采集 | ✅ 完成 | bsp_adc.c，约12.82kHz 采样，64 点/批 |
...
| app_predict.h | ✅ 完成 | `predict_param_t` + `P` 全局 const 实例（19 个参数） |
...
### 🟠 HIGH-02: 温度传感器故障检测缺失 — 已修复

**位置**: `modules/app/app_acquire.c` 第 67~83 行  
**现象**: 拔掉 NTC 热敏电阻后，串口仍显示"正常"温度值，无法触发传感器故障告警  
**根因**: 
- 当前代码只检测了**电压传感器**故障（`v_avg <= 10 || v_avg >= 4085`）
- **没有检测温度传感器故障**
- NTC 开路时，ADC 引脚悬空，读到的值可能是**中间值**（如 2048），不是极端值
- 这个中间值被 `calculate_temp()` 转换为"正常"温度（如 25℃），不会触发故障

**影响**:
- NTC 断线/接触不良时，系统无法检测到传感器故障
- 故障注入测试中 `FAULT_NTC_OPEN` 无法通过硬件验证
- 违反安全关键系统设计原则：传感器失效必须被检测
...
1. 拔掉 NTC 热敏电阻
2. 观察串口输出
3. 预期：`ALM:3, FC:3`（HARDFAULT + 传感器故障）
