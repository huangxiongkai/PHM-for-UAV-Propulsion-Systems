...
| `Logic_thread3` | 9 | 1536B | 事件驱动 | 6 阶段特征提取链 + 传感器自检 |
...
> 以下结果来自 MCU 台架上的 **Testbench 故障注入测试**（编译期宏启用，将真实 ADC 码替换为仿真故障输出），
> 用于验证实时检测链路与状态机响应性能，**不代表真实飞行环境数据**。

### 快速掉压注入测试（EXP02，-5V/s）

| 指标 | 预期 | 实测 | 判定 |
|:---|:---|:---|:---|
| Detection Latency | < 1s | 960ms | 通过 |
| Early Warning（提前量） | > 0 | 649ms | 通过 |
| Total Response | < 2s | 1.6s | 通过 |
| Alarm Output Time | < 1ms | 145µs | 通过 |

Detection 960ms 分解：IIR 收敛约 530ms + V_Risk 累积约 430ms + FSM 条件保持 200ms（HI<80 需连续 10 帧）。

### 线程实时性（72MHz，DWT Cycle Counter 计时，精度 1µs）
...
| Actuator | < 200µs | 145µs | 149µs |

### 栈使用率（阈值 < 50%）

rawdata_thread1 峰值 508/1024B（49%）为全系统最高，其余线程 25%~41%，全部达标。

完整实验报告见 [docs/EXP01~EXP05](docs/)（掉压 / 温升 / ADC 卡死共 5 组）。
...
- 当前系统仅独立完成监测与分级声光告警，**不控制主飞控、电机或 ESC**
- 油门接口为占位实现（恒返回 1000），油门联动 V_ref 冻结分支不会被触发，未接入真实飞控链路
- 实验结果来自台架故障注入，尚未在真实飞行环境中验证
- 部分阈值参数（滞回裕量、风险权重）仅在小样本工况下整定，需更多工况标定
