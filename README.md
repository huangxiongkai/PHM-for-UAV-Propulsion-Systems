...
**Safety Copilot** — 面向**工业巡检、特种物流、高负载无人机作业**等严苛场景的嵌入式电池健康监测系统。通过多传感器融合与多级滞回状态机，实现电池热失控与欠压故障的**提前预警**（3.5s~5.0s）与**主动防护**。系统运行在 STM32F103C8 上，采用 RT-Thread Nano 实时操作系统，与主飞控物理/逻辑隔离，符合航空电子"故障安全"规范。

### 核心创新点

- **独立模块设计**：与主飞控物理/逻辑隔离，符合航空电子"故障安全"规范
- **分层 PHM 架构**：对齐 PHM Society 标准分层结构（数据采集→特征提取→健康评估→决策执行）
- **主动防护闭环**：通过 SBUS 总线串联拦截，执行保姿态动态降级，100% 保留姿态闭环（Roll/Pitch/Yaw），仅限制总能量
- **三重底层防御**：硬件 RC 滤波 + 猝发过采样 + 原始码熔断，信噪比提升 12dB，误报率 <0.5%
...
### 1. 数据采集层三重防御 (app_acquire.c)

Acquire 线程实现三重底层防御机制，确保原始数据质量：

| 防御层 | 具体实现 | 技术参数 |
|--------|----------|----------|
| **硬件滤波** | RC 低通滤波器 | R=1kΩ, C=2.2μF, fc≈72Hz |
